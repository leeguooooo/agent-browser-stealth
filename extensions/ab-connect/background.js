...
async function reannounceAttachedTabs() {
  for (const [tabId, tab] of tabs.entries()) {
    ...
    try {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'OpenClaw Browser Relay (auto-attach enabled)',
      })
      continue
    }
...
async function connectOrToggleForActiveTab() {
  // Click now triggers connect + auto-attach all tabs.
  cancelReconnect()
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err)
    console.warn('connect failed', message, nowStack())
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => void whenReady(async () => {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err)
    console.warn(`Auto-attach tab ${tabId} on update failed: ${message}`)
  } finally {
    tabOperationLocks.delete(tabId)
  }
}))
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  ...
  // Auto-attach any unattached tabs while relay is healthy.
  if (relayWs && relayWs.readyState === WebSocket.OPEN) {
    await autoAttachAllTabs()
  }
